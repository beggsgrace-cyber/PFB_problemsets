#!/usr/bin/env python

import os, sys
import math
from sequence_to_kmer_list import *
from fastq_file_to_sequence_list import *


## method: count_kmers(kmer_list)
##
##  Counts the frequency of each kmer in the given list of kmers
##
##  input parameters:
##
##  kmer_list : list of kmers (type: list)
##               ie.  ["GATC", "TCGA", "GATC", ...]
##
##
##  returns kmer_counts_dict : dict containing ( kmer : count )
##                    ie.  {  "GATC" : 2,
##                            "TCGA" : 1,
##                             ...       }


def count_kmers(kmer_list):

    kmer_count_dict = dict()

    ##################
    ## Step 2:
    ## begin your code
   
    for kmer in kmer_list:
        if kmer in kmer_count_dict:
            previous_count = kmer_count_dict[kmer]
            new_count = previous_count + 1
            kmer_count_dict[kmer] = new_count
        else:
            kmer_count_dict[kmer] = 1






    ## end your code
    ################

    return kmer_count_dict

def kmer_entropy(kmer_list):
    kmer_entropy_dict = dict()
    for kmer in kmer_list:
        pA = kmer.count('A')/len(kmer)
        pC = kmer.count('C')/len(kmer)
        pT = kmer.count('T')/len(kmer)
        pG = kmer.count('G')/len(kmer)
        if pA != 0 and pC != 0 and pT != 0 and pG != 0:
            kmer_entropy_dict[kmer] = - ((pA * math.log2(pA))+(pC * math.log2(pC))+(pT * math.log2(pT))+(pG * math.log2(pG)))
    return kmer_entropy_dict

def main():

    progname = sys.argv[0]

    usage = "\n\n\tusage: {} filename.fastq kmer_length num_top_kmers_show\n\n\n".format(
        progname
    )

    if len(sys.argv) < 4:
        sys.stderr.write(usage)
        sys.exit(1)

    # capture command-line arguments
    fastq_filename = sys.argv[1]
    kmer_length = int(sys.argv[2])
    num_top_kmers_show = int(sys.argv[3])

    seq_list = seq_list_from_fastq_file(fastq_filename)

    all_kmers = list()

    #######################
    ## Step 1:
    ## begin your code, populate 'all_kmers' list with the
    ## collection of kmers from all sequences

    for seq in seq_list:
        kmer_list = sequence_to_kmer_list(seq, kmer_length)
        all_kmers.extend(kmer_list) #extend adds values from one list to another list; append will make a list of lists




    ## end your code
    #######################

    kmer_count_dict = count_kmers(
        all_kmers
    )  # see step 2 above. You implement this. :-)

    unique_kmers = list(kmer_count_dict.keys())
    entropy_dict = kmer_entropy(unique_kmers)
    
    #########################
    ## Step 3: sort unique_kmers by abundance descendingly
    ## (Note, you can run and test without first implementing Step 3)
    ## begin your code       hint: see the built-in 'sorted' method documentation

    #need to overwrite unique_kmers
    sorting = sorted(kmer_count_dict.items(), key=lambda item: item[1], reverse=True)
    new_dict = dict(sorting)
    unique_kmers1 = list(new_dict.keys())
    
    sorting_by_entropy = sorted(entropy_dict.items(), key=lambda item: item[1], reverse=True)
    new_ent_dict = dict(sorting_by_entropy)
    unique_kmers = list(new_ent_dict)



    
    #

    ## end your code

    ## printing the num top kmers to show
    top_kmers_show = unique_kmers[0:num_top_kmers_show]

    for kmer in top_kmers_show:
        print("{}: {}, {}".format(kmer, kmer_count_dict[kmer], entropy_dict[kmer]))

    sys.exit(0)  # always good practice to indicate worked ok!


if __name__ == "__main__":
    main()
